...
@Component({
  selector: 'app-floor-and-trip',
  templateUrl: './floor-and-trip.component.html',
  styleUrls: ['./floor-and-trip.component.scss']
})
export class FloorAndTripComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.$self.rxchartRawData.subscribe((res: IFchartRawData) => {
      ...
      if (res && res.data && url.includes('floorTrip')) {
  ...
  onAccumulateData() {
     ...
     */
    this.DatasetsConfig = [
      { label: 'No of floors', backgroundColor: '#78BE20', data: [], maxBarThickness: 40, columnName: 'floor' },
      { label: 'No of trips', backgroundColor: '#460dc1', data: [], maxBarThickness: 40, columnName: 'trip' },
    ];
    ...
    this.DatasetsConfig.forEach((ele: IFChartExtendDatasets) => {
      this.barChartLabels.forEach(label => {
        ...
        const colVal = val && val[ele.columnName] ? val[ele.columnName] : 0;
    ...
    this.barChartOptions = {
      ...
      scales: {
        yAxes: [{
          ...
          scaleLabel: {
            display: true,
            labelString: 'Number of floors & trips',
